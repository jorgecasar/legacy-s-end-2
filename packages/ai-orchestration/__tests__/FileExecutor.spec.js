...
import { FileExecutor } from "../src/domain/services/FileExecutor.js";
  ...
  <file path="src/oldFile.js" operation="delete">
	...
	test("should correctly apply changes to the filesystem", async () => {
		...
		const file2 = await fs.readFile(
			path.join(testDir, "nested/dir/test2.txt"),
			"utf-8",
		);
		...
		const file1Updated = await fs.readFile(
		...
		await assert.rejects(
			async () => await fs.access(path.join(testDir, "test.txt")),
			/ENOENT/,
		);
	});
	...
	test("should prevent directory traversal attacks", async () => {
		const changes = [
			{ path: "../outside.txt", operation: "create", content: "bad" },
		];
