...
describe("Execution Validation: AI Reviewer (review-pr)", () => {
  test("should execute successfully with mocked environment", () => {
    ...
    assert.ok(
      stdout.includes("--- AI CALL SIMULATION (REVIEWER) ---"),
      "Should have triggered reviewer simulation",
    );
    assert.ok(
    ...
    assert.ok(
      stdout.includes("[COST REPORT SIMULATION] Updating table with: | Reviewer |"),
