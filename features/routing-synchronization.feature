Feature: Routing Synchronization
  ...
  Scenario: URL updates when entering a quest
    ...
    Then the URL should match "/quest/alarions-awakening/chapter/0"
    And the Game Level should be visible

  Scenario: URL updates when advancing chapters
    Given I am in the quest "alarions-awakening" at chapter 0
    When I advance to chapter 1
    Then the URL should match "/quest/alarions-awakening/chapter/1"

  Scenario: Direct access via URL
    When I navigate directly to "/quest/alarions-awakening/chapter/1"
